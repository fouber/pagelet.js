	...
	pagelet.load = function(url, pagelets, pageletOptions, callback){
		if(pagelets && pagelets.length){
			...
			if ( _xhr && _xhr.readyState < 4) {
				_xhr.onreadystatechange = noop;
				_xhr.abort();
			}

			_xhr = new global.XMLHttpRequest();
			//_xhr.onprogress = progress;
			_xhr.onreadystatechange = function(){
				if(_xhr.readyState == 4) 
				{
					...
					if((_xhr.status >= 200 && _xhr.status < 300) || _xhr.status == 304){
			            ...
			            callback(_xhr.statusText || (_xhr.status ? 'error' : 'abort'));
			        }
	...
	document.documentElement.addEventListener( 'click', function(e) 
	{
		...
		while(target && target.tagName.toLowerCase()!="body")
		{
			if( target.tagName.toLowerCase() === 'a' ) {
				...
				//to array
				var pagelets = (target.getAttribute('data-pagelets') || '').split(/\s*,\s*/).filter(filter)
